...
class CoughAnalysisHelper {
  ...
  static Map<String, dynamic> analyzeCoughPattern(
    ...
    final estimatedCoughCount = (duration / 3).ceil(); // ~1 toux toutes les 3 secondes
    
    // Type de toux (basé sur mots-clés)
    String coughType = 'sèche';
    if (lowerText.contains('glaire') || lowerText.contains('crachat') || lowerText.contains('expectoration')) {
    ...
    if (lowerText.contains('sang') || lowerText.contains('hémoptysie')) tbRisk += 40;
    if (intensityScore >= 2) tbRisk += 10;
    
    // CRITÈRES PNEUMONIE
    if (coughType == 'productive' || coughType == 'grasse') pneumoniaRisk += 35;
    if (lowerText.contains('douleur') || lowerText.contains('thoracique')) pneumoniaRisk += 30;
    if (lowerText.contains('fièvre') || lowerText.contains('chaud')) pneumoniaRisk += 20;
    if (intensityScore == 3) pneumoniaRisk += 15;
    
    // Recommandations
    String recommendation;
    if (tbRisk > 70 || pneumoniaRisk > 70) {
      recommendation = '🚨 URGENCE: Consultation médicale immédiate + Test GeneXpert (TB) ou Radiographie (Pneumonie)';
    } else if (tbRisk > 40 || pneumoniaRisk > 40) {
      recommendation = '⚠️ ALERTE: Consulter un médecin dans 48h + Surveillance SpO2';
    } else if (hasCoughKeywords && intensityScore >= 2) {
      recommendation = '💊 Toux modérée: Repos, hydratation, suivi RespiraBox quotidien';
    } else {
      recommendation = '✅ Toux légère: Hydratation, repos. Surveillance si persistance >3 jours';
    }
    ...
    return {
      ...
      'medicalScore': {
        ...
        'urgencyLevel': tbRisk > 70 || pneumoniaRisk > 70 ? 'high' : 
                       tbRisk > 40 || pneumoniaRisk > 40 ? 'medium' : 'low',
      }
    };
  }
}
